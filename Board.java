...
public class Board implements Ilayout, Cloneable {
	...
	public List<Ilayout> children() {
		...
		if(movesAvailable.size() == 9) {
			...
			while(!array.isEmpty()) {
				d = this.getDeepCopy();
				int move = r.nextInt(array.size());
				d.movePlayed = array.get(move);
				array.remove(move);
				d.move(d.movePlayed);
				c.add(d);
				d=null;
    ...
    @Override
   	public Ilayout randomChild() {
		...
		Random r = new Random();
		d = this.getDeepCopy();
		int[] array = new int[movesAvailable.size()];
		int i = 0;
		for(int move : getAvailableMoves()) {
			array[i] = move;
			i++;
		}
		d.movePlayed = array[r.nextInt(array.length)];
		d.move(d.movePlayed);
    	return d;
    }

	@Override
	public boolean statusGameOver() {
		if(gameOver == true)
			return true;
		return false;
	}
